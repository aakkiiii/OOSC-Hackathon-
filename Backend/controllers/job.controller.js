...
export const getAllJobs = async (req, res) => {
  try {
    ...
    if (!jobs) {
      return res
        .status(404)
        .json({ message: "No Scholarship found", status: false });
    }
...
export const getJobById = async (req, res) => {
  try {
    ...
    if (!job) {
      return res
        .status(404)
        .json({ message: "Scholarship not found", status: false });
    }
...
export const getRecommendedJobs = async (req, res) => {
  try {
    ...
    try {
      const systemPrompt =
        "You are a Scholarship recommendation engine. From the candidate jobs, pick the ones most related to the target Scholarship (similar role, skills, location or type). Reply ONLY with a JSON array of the matching candidate ids, ordered best first. No extra text.";
